...
int NghienCuuVien::NumProjects() const {
	int count = 0;
	for (const string& s : maDuAn) {
		if (!s.empty() && s[0] == 'D') count++;
	}
	return count;
}

void NghienCuuVien::Nhap(istream& inDev) {
	NhanSu::Nhap(inDev);
...
void NghienCuuVien::Xuat(ostream& outDev) const {
	NhanSu::Xuat(outDev);
	outDev << "\n==== THONG TIN NGHIEN CUU VIEN ==== \n";
