...
NhanSu* NhanSu::getHighestSalary(const vector <NhanSu*>& list) {
	NhanSu* tmp = nullptr;
	double maxx = 0;
	for (int i = 0; i < list.size(); ++i) {
		if (maxx < list[i]->getSalary()) {
			maxx = list[i]->getSalary();
			tmp = list[i];
		}
	}
	return tmp;
...
istream& operator >> (istream& inDev, vector <NhanSu*>& listNhanSu) {
	...
	for (int i = 0; i < n; ++i) {
		string s;
		cout << "Role of person " << i + 1 << " (giang vien/tro giang/nghien cuu vien/nghien cuu sinh/chuyen vien): ";
		getline(inDev >> ws, s);
		NhanSu::chuanHoa(s);
...
ostream& operator << (ostream& outDev, const vector<NhanSu*>& listNhanSu) {
	...
	outDev << "\n==> TOTAL SALARY <==  " << NhanSu::totalSalary(listNhanSu) << "\n";
